<template>
    ...
    <div class="goods-wapper">
      <ul class="type-box">
        <li class="type-item" v-for="(item,index) in goodClassList" :key="index" :class="item.selected?'selected':''" @click="selectedMethod(index)">
            <a v-text="item.name"></a>
        </li>
      </ul>
    </div>
<!--右部分@scroll="goodsScroll();"-->
<div class="goods-list" id='goods' >
      <div class="goods-item" @scroll="goodsScroll()" v-for="(goodItem,idx) in goodClassList" :key="idx"   :id='"goodItem"+idx'>
        ...
        <ul class="foods-list">
          ...
          <li v-for="(foodItem,index) in foods" :key="index">
            <div class="food-img" @click="selecedFood(idx+'-'+index,foodItem)">
              <img v-bind:src="foodItem.img_url" />
            </div>
            <div class="food-context">
              ...
              <div class="food-description">
                <span>月售{{foodItem.sales}}</span>
                <span style="margin-left: 8px">好评率{{foodItem.id}}%</span>
              </div>
...
export default{
	data(){
     ...
     foods:"",
      goodClassList:[
        {"id":1,"name":"热销榜","selected":""},
        {"id":2,"name":"单人套餐","selected":""},
        {"id":3,"name":"限时特惠","selected":""},
        {"id":4,"name":"精选热菜","selected":""},
        {"id":5,"name":"爽口凉菜","selected":""},
        {"id":6,"name":"小吃主食凉菜","selected":""},
        {"id":7,"name":"限时特惠","selected":""},
        {"id":8,"name":"精选热菜","selected":""}
  ...
  methods:{
    getdetail(){
			this.$http.get("goods/bolist?nid=1").then(result=>{
				this.foods=result.body.msg
    ...
    addFoods(idx, item) {
      ...
      if (product) {
        product.count++;
        this.$store.commitcommit("setProductArray", product);
      } else {
